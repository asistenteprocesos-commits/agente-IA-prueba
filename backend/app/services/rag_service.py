...
Estrategia:
  1. Embeddings: genera vectores con Ollama (qwen3-embedding) si está disponible,
     o usa TF-IDF como fallback determinístico sin dependencia de GPU.
  2. Índice vectorial: FAISS en disco (storage/vector_store/).
     Si no hay FAISS instalado, usa coseno sobre numpy como fallback.
  3. Resultado: lista de fragmentos ordenados por relevancia con cita de fuente.

La búsqueda TF-IDF es suficiente para prototipos; en la PC de producción
con Ollama disponible se activan los embeddings reales.
"""
...
class RAGService:
    ...
    def buscar(
        ...
        # Intentar embeddings con Ollama, caer a TF-IDF si no está disponible
        try:
